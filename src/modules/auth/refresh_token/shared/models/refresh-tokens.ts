...
export interface Refresh_Tokens_Props {
  identity_id: string;
  token_hash: string;
  expires_at: Date;
  revoked_at: boolean;
...
export class Refresh_Tokens {
  private _id: string;
  private props: Refresh_Tokens_Props;

  constructor(
    props: Replace<
      Refresh_Tokens_Props,
      {
        created_at?: Date;
        updated_at?: Date;
      }
  ...
  public get revoked_at(): boolean {
    return this.props.revoked_at;
  }

  public set revoked_at(revoked_at: boolean) {
    this.props.revoked_at = revoked_at;
  }
